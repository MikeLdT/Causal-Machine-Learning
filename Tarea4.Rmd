...
#Ilustramos las tablas de balance
br<-readRDS("br.rds")
bt<-readRDS("bt.rds")
...
modelo_9_val <- gamlr(y = resid_monto_val, x = x_val, family = "gaussian")
```


```{r}
# Esta es la matriz de la t de tratamiento en lugar de los residuales
x_prueba <- sparse.model.matrix(~ mailing_indicator*.+ 0, 
                         data_9 %>% 
                    select(-resid_monto, -k, -customer_id, -training_sample, 
                           -treat_hat, -monto_hat, -resid_treat))


# CATE entrenamiento
cate <- x_prueba[, grepl("mailing", colnames(x_prueba))] %*% unlist(coef(modelo_9))[grepl("resid", rownames(coef(modelo_9))),]

# CATE validacion
cate_val <- x_val[, grepl("resid", colnames(x_val))] %*% unlist(coef(modelo_9_val))[grepl("resid", rownames(coef(modelo_9_val))),]

cate_val <- as.data.frame(as.matrix(cate_val))
#cate <- predict(modelo_9, newdata = x)
lab1 <- paste0("Media/Val=", round(mean(cate_val$V1), 2))
...
cate %>% 
  ...
  geom_histogram(color = "black", data = cate_val, 
                 aes(y = ..density.., fill = "Validation"), alpha = 0.5, binwidth = 10) +
  geom_vline(aes(xintercept = mean(cate_val$V1), color = "Media/Val"), linetype = "dashed") +
  geom_vline(aes(xintercept= mean(cate$V1), colour = "Media/Training"), linetype = "dashed") +
  labs(x = "CATE", y = "Densidad", fill = "Base") +
  scale_color_manual(name = "Estadísticos", 
                     values = c("Media/Training" = "red",  "Media/Val" = "blue")) +
  xlim(-2, 2) +
  theme_minimal()
