...
# x_prueba_val <- sparse.model.matrix(~ mailing_indicator*.+ 0, 
#                          base_validacion %>% 
#                       select(-resid_monto, -customer_id, -training_sample, -media_monto, 
#          -media_treat) %>% 
#            bind_cols(mailing_indicator = treat_validacion))

# CATE entrenamiento
cate <- x[, grepl("mailing", colnames(x_prueba))] %*% unlist(coef(modelo_9))[grepl("resid", rownames(coef(modelo_9))),]

# CATE validacion
cate_val <- x_val[, grepl("resid", colnames(x_val))] %*% unlist(coef(modelo_9))[grepl("resid", rownames(coef(modelo_9_val))),]
