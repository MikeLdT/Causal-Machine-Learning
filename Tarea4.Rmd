...
```{r, cache=TRUE}
setwd("/Users/adrian_martinez/Dropbox/Maestría/Maestría Clases/Segundo_Semestre/economia_computacional/Causal-Machine-Learning")
...
# Aquí tienen que dejar solo las más correlacionas, es algo que dijo isidoro en
# clase para que se pueda ver bien el mapa...
...
x <- sparse.model.matrix(~ mailing_indicator*.+ 0 - 
                           resid_treat:mailing_indicator - mailing_indicator, 
                         data_9 %>% 
...
cate %>% 
  ggplot(data = ., aes(x = seg100)) +
  geom_histogram(color = "black", fill = "lightblue", aes(y = ..density..)) +
  xlim(-2, 2)
```
