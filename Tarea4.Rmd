...
```{r, cache=TRUE}
setwd("C:/Users/georg/Documents/ITAM/Maestría/2 Semestre/Economía Computacional/Tarea_4")
...
crm<-crm %>% 
  select(-customer_type_3, -orders_online_attributed_target, -acquisition_months_since, -in_database_months, -emails_days_2yr, -emailview_6m)
...
```{r}
#ASUMIENDO QUE VAR DEP 
attach(crm)
ydep<- crm$outcome_spend


Mod1<-lm(ydep~mailing_indicator)
...
library(minDiff)
assigment <- create_groups(as.data.frame(base_training), 
                           criteria_scale = c("mailing_indicator"), sets_n = 5, repetitions = 1000)
```

Nuestro intento raro:
```{r}
#Quitar variable depenediente de controles X
lasso_treat <- vector(mode="list", length=5)
lasso_monto <- vector(mode="list", length=5)

X<-assigment %>% 
  mutate(k=newSet) %>% 
  select(-outcome_spend, -mailing_indicator, -newSet)

trat<-assigment %>% 
  mutate(k=newSet) %>% 
  select(mailing_indicator,k)
...
modelo<-map_dfr(1:5,
                function(a) {
                  treat_fit <-gamlr(x = X[newSet!=a, , drop= F],
                              y = trat[newSet!=a], family="binomial") 
                  treat_hat<-as.numeric(predict(treat_fit, newdata = X[newSet==a, , drop= F], type = "response"))
                  lasso_treat[a]<-treat_fit
                  
                  monto_fit <-gamlr(x = X[newSet!=a, , drop= F],
                                    y =  monto_ret[newSet!=a])
                  monto_hat<-as.numeric(predict(monto_fit,newdata = X[newSet==a, , drop= F],type ="response"))
...
modelo<-map_dfr(1:5,
                function(a) {
                  ...
                  treat_hat<-as.numeric(predict(treat_fit, newdata = X[k==a, , drop= F], type = "response"))
                  lasso_treat[[a]]<<-treat_fit
                  
                  monto_fit <-gamlr(x = X[k!=a, , drop= F],
                                    y =  monto_ret[k!=a])
                  monto_hat<-as.numeric(predict(monto_fit,newdata = X[k==a, , drop= F],type ="response"))
...
stargazer(ef_prom_1,ef_prom_2,type="text")
```
